...
- **DuckDB** (`data/historic_fundamentals.duckdb`) — monthly PE/P/FCF/EV/EBITDA timeseries, valuation stats, sector/industry aggregates (43K rows), analyst estimates, the monthly DCF cache (`dcf_results`) and its append-only snapshot history (`dcf_results_history`, one dated row per ticker per rebuild — the only record of what the DCF said at a past date)
...
- `DcfResult.warnings` carries a list of data quality messages (e.g. zero market cap, WACC below 5%, cost of debt below the risk-free rate, terminal growth clamped). Shown as amber banners in the UI.
...
- **Agentic AI DCF Valuator** (`api/ai_dcf_router.py`, data layer in `api/ai_dcf_data.py`) — a second, independent DCF source: three evidence sub-agents (Fundamentals Historian, Industry & Competitors, Guidance & MD&A) feed a senior DCF Architect persona that authors per-year bear/base/bull revenue growth, margin, and capex assumptions grounded in company history, industry trends, competitor transcripts, and multi-year MD&A/guidance. The LLM only authors assumptions; the same deterministic `dcf.run_dcf_av` engine that powers the mechanical scenarios computes the actual valuation. Standalone endpoints (`GET /research/ai-dcf{,/status}`, `POST /research/ai-dcf/cancel`) plus an in-process, cache-aware entry point (`get_or_run_ai_dcf`) shared with the AI Research tab above. Guardrails on the Architect's authored assumptions are advisory (never silently clip an authored number) and, for capex, ticker-anchored — each company is checked against its own historical capex range rather than one flat percentage for every sector, since capital intensity varies enormously by industry (e.g. a regulated utility's normal range sits well above what a flat cap calibrated for asset-light sectors would allow). Full design rationale, architecture, and phased build record (including real bugs found and fixed during live verification, most recently a cost-of-debt-vs-risk-free-rate sanity check and the ticker-anchored capex fix): `features/ai_dcf/SPEC.md`, `features/ai_dcf/PLAN.md`, `features/ai_dcf/PLAN_GUARDRAILS.md`.
