#!/usr/bin/env python3
"""
Maps balance sheet XBRL concepts to line items using AI.

Usage:
    uv run map_bs_concepts.py
"""
import os
...
def main():
    ...
    #### THIS CODE IMPORTS MAPPINGS PREVIOUSLY EXPORTED FROM THE COMCEPTS DATABASE
    #### USING THE COMMAND export_mappings.py
    ### AND CREATES A LIST OF DICTIONARIES
    with open("data/balance_concept_mappings.txt", "r") as f:
    ...
    balance_mapping_json = json.dumps(BALANCE_SHEET_MAPPING)
    ...
    for concept in concepts_list:
        ...
        a JSON file with predifined balance sheet statement line items. 
        Your job is to map the given XBRL concept to the best match balance sheet statement line item in the 
        predifined mapping JSON file. Return only the balance sheet statement line item that 
        best matches the given XBRL concept. No extra verbage.
        DO NOT RETURN ANYTHING OTHER THAN THE balance sheet STATEMENT LINE ITEM. NO EXTRA VERBAGE. NO EXTRA TEXT. NO EXTRA COMMENTS.
        NO BLOCK CODE. 

        JSON file: {balance_mapping_json}
        ...
        if LLM1_response != LLM2_response:
            ...
            decider_instructions = f""" You are a finance analyst and expert with SEC EDGAR filings and XBRL concepts.
                Your job is to map the given XBRL concept to the best match balance sheet statement line item in the 
                provided predifined mapping JSON file. You will be given the mappings for a given XBRL concept mapped by two 
    ...
    final_response_dicts = [
        {"concept": concept, "balance_mapping": response}
