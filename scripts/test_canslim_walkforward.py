#!/usr/bin/env python3
"""
Out-of-sample stability check for the CANSLIM-augmented composite
(scripts/test_canslim_augmented_ab.py showed a full-period Sharpe LOSS of
0.952 -> 0.893 from adding the 7 CANSLIM sub-factors; this checks whether
that loss is fold-consistent or an artifact of one bad sub-period).

Same fold structure as scripts/test_greenblatt_walkforward.py: train=5y/
test=1y non-overlapping annual test windows walking across the full history.
No fitted parameters in the composite (pure cross-sectional rank/z-score each
month), so "out-of-sample" here means period-consistency, not train/test
leakage.

Usage:
    uv run scripts/test_canslim_walkforward.py
"""
...
def main() -> None:
    ...
    print("\n" + "=" * 90)
    print("WALK-FORWARD FOLD STABILITY — composite_baseline vs composite_augmented (CANSLIM)")
