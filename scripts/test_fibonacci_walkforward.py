#!/usr/bin/env python3
"""
Out-of-sample stability check for the Fibonacci-augmented composite
(scripts/test_fibonacci_factors.py section 4 showed a full-period Sharpe LOSS
of 0.952 -> 0.873 from adding fib_618_proximity; this checks whether that loss
is fold-consistent or an artifact of one bad sub-period).

Same fold structure as scripts/test_greenblatt_walkforward.py /
test_canslim_walkforward.py: train=5y/test=1y non-overlapping annual test
windows walking across the full history. No fitted parameters in the
composite (pure cross-sectional rank/z-score each month), so "out-of-sample"
here means period-consistency, not train/test leakage.

Usage:
    uv run scripts/test_fibonacci_walkforward.py
"""
...
def main() -> None:
    ...
    if "fib_618_proximity" not in raw.columns:
        log.error("Run scripts/backfill_fibonacci_factors.py first.")
        sys.exit(1)
    ...
    universe["composite_augmented"] = _composite(universe, extra_quality=["fib_618_proximity"])
    ...
    print("\n" + "=" * 90)
    print("WALK-FORWARD FOLD STABILITY — composite_baseline vs composite_augmented (Fibonacci)")
