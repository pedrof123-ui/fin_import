#!/usr/bin/env python3
"""
Backfill dividends table in av_financials.duckdb.

Fetches DIVIDENDS from Alpha Vantage for all tickers (or a subset) and stores
the per-share dividend history (ex_date, declaration, record, payment, amount).
...
After running, re-run hf_update.py --skip-estimates to refresh dividend_yield in monthly_pe.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch dividends for all tickers.")
...
def main() -> int:
    ...
    log.info("Fetching dividends for %d ticker(s)", len(tickers))

    already_have: set[str] = set()
    if not args.force:
        rows = db.conn.execute("SELECT DISTINCT ticker FROM dividends").fetchall()
