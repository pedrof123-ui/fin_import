...
def get_ticker_latest_quarters(av_conn: duckdb.DuckDBPyConnection, ticker: str | None) -> list[tuple[str, date]]:
    """Return [(ticker, latest_fiscal_date_ending)] from av_financials."""
    if ticker:
...
def main() -> int:
    ...
    for symbol, latest_date in tqdm(ticker_dates, desc="Backfill", unit="ticker"):
        quarters = fiscal_date_to_quarters(latest_date, today, args.quarters)
