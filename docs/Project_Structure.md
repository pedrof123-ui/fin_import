...
│   ├── live_scores_YYYYMMDD.csv         Phase 8 output: ranked investable universe with scores, risk flags, missing-data counts (generated by score_live.py)
...
│   │                                    PORTFOLIO_CONFIGS: top 20%, top 10%, top 50, top 25. Equal-weight. TC-adjusted.
...
scripts/ also contains:
    ...
    score_live.py                        Live scoring: load latest PIT-safe features, apply universe filters, rank by score, write docs/live_scores_YYYYMMDD.csv
                                         Options: --top N, --verbose, --output PATH, --model PATH
```
