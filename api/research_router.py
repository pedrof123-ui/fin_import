...
Earnings & MD&A Historian / Technical Analyst sub-agents feeding a Chief Analyst synthesis) via
OpenRouter; cached 24h in research_cache.duckdb.
"""
...
class ValuationOutput(BaseModel):
    ...
    @field_validator("dcf_intrinsic_value", mode="before")
    @classmethod
...
def render_to_markdown(
    ...
    direct_competitors_md: str = "", market_share_md: str = "",
) -> str:
    h = report.header
    v = report.valuation
    lines: list[str] = []
...
def render_valuation_model_tables(
    ...
    """Assembles the four Valuation Model Detail tables. Every number here is computed in
...
def _build_post_subagent_tables(ticker: str, valuation_out, competitive_out) -> tuple[str, str, str]:
...
def _format_specialist_outputs(
    ...
    return (
        ...
        f"dcf_intrinsic_value: {valuation.dcf_intrinsic_value}\n"
...
def _validate_report(
    ...
    findings are logged and surfaced as a QC footer on the rendered report."""
    findings: list[str] = []
...
async def _run_research_agent(
...
) -> tuple[EquityResearchReport, list[str], str, str, str]:
    ...
    log.info("[%s] research: data gathering done in %.1fs — calling sub-agents", ticker,
    ...
    return report, findings, valuation_tables_md, direct_competitors_md, market_share_md
...
async def _background_generate(ticker: str, model: str):
    ...
    try:
        ...
        markdown = render_to_markdown(report, valuation_tables_md, direct_competitors_md, market_share_md)
        if findings:
